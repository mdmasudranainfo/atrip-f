...
const BlogDetails = async (context: {
  ...
  try {
    const res = await fetch("http://localhost:8000/api/news/15", {
      method: "GET",
      headers: {
    ...
    <div>
      ...
      <div className="container mx-auto py-10 flex  gap-5">
        ...
        <div className="w-[300px] mt-5 sticky top-0 h-fit hidden md:block">
          ...
          <ul>
          {
            parsedContent?.map((item: any, index: number) => {
              return <li key={index}>
                <a href={`#${index + 1}`} className="text-sm text-blue-500 cursor-pointer my-2"> {index + 1}. {item.title}</a>
              </li>
            })
          }
        ...
        <div className="">
          {
            parsedContent?.map((item: any, index: number) => {
              return <div key={index} id={`${index + 1}`}>

                <h2 className="text-2xl font-bold py-2 text-center">{item.title}</h2>
                <div className="flex justify-center ">
                  <Image src={item.image} alt={item.title} width={500} height={500} layout="responsive" className="rounded-lg  overflow-hidden" />
                </div>
